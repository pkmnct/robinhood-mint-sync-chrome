...
function populateMultipleAccountFields(multipleAccounts: Array<{ robinHoodAccountName: string }>): void {
  ...
  multipleAccounts.map((account, index) => {
    ...
    clonedRobinhoodInput.addEventListener("change", function () {
      ...
      multipleAccountInputChange(index, "robinHoodAccountName", value);
    });
  });
 ...
 */
function multipleAccountsAddAction(): void {
  // Disable add button
 ...
 */
function passedValidation(value: string): boolean {
  if (!value) {
    return false;
  }
 ...
 */
function multipleAccountsDeleteAction():void {
 ...
 */
function multipleAccountInputChange(index: number, key: string, value: string): void {
  chrome.storage.sync.get({ multipleAccounts: [] }, (result) => {
    // Bail with bad data
